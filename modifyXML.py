...
def getReleaseBranchName(projectPath):
  ...
  return version + '（'+ yiuanName +'）'

def modifyProjectNameFromProject(projectPath):
  print(projectPath)
  # 获取项目名称
  stringsTree = read_xml(projectPath + "/res/values/strings.xml")
  stringsNode = find_nodes(stringsTree, "./string")
  ...
  apkName = "移动护理-" + yiuanName + "V" + version

  # 修改打包名称
  tree = read_xml("./copyfiles/build.xml")
  buildNodes = find_nodes(tree, ".")
  for node in buildNodes:
    node.set("name", apkName)
  pass

  write_xml(tree, "./copyfiles/build.xml")

# def modifyProjectNameFromLocalFiles(apkName="移动护理"):
#   # 获取项目名称
#   stringsTree = read_xml("./diffProjectFiles/strings.xml")
#   stringsNode = find_nodes(stringsTree, "./string")
#   result_stringNodes = get_node_by_keyvalue(stringsNode, {"name": "yiyuan_mingcheng"})
#   for node in result_stringNodes:
#     yiuanName = node.text
...
#
#   # 获取版本号
#   manifestTree = read_xml("./diffProjectFiles/AndroidManifest.xml")
#   manifestNodes = find_nodes(manifestTree, ".")
#   for node in manifestNodes:
#     value = node.attrib["{http://schemas.android.com/apk/res/android}versionName"]
#     # for attr in node.attrib:
